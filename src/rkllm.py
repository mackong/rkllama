...
"""
    ...
    def _get_prediction_point(self, attn_scores, n_width, n_height, activation_threshold=0.3):
        """从注意力分数计算最佳预测点。简化版的 get_prediction_region_point。"""
        ...
        # 为了简化，直接使用所有激活点的加权平均值，效果通常足够好
        scores = attn_scores[0][valid_indices]
        total_score = torch.sum(scores)

        y_coords = valid_indices.float() // n_width
        x_coords = valid_indices.float() % n_width

        center_x = torch.sum(((x_coords + 0.5) / n_width) * scores) / total_score
        center_y = torch.sum(((y_coords + 0.5) / n_height) * scores) / total_score
