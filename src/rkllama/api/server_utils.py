...
class GuiActorEndpointHandler(EndpointHandler):
    ...
    def handle_complete(cls, modele_rkllm, model_name, prompt, image, label):
        ...
        # Get vision encoder and pointer head paths from model directory
        model_dir = os.path.join(rkllama.config.get_path("models"), model_name)
        vision_encoder_path = os.path.join(model_dir, "vision_encoder.rknn")
        pointer_head_path = os.path.join(model_dir, "pointer_head.rknn")

        # Check if model files exist
        if not os.path.exists(vision_encoder_path) or not os.path.exists(pointer_head_path):
            logger.error(f"GUI Actor models not found in {model_dir}")
            return jsonify({
                "error": "GUI Actor models (vision_encoder.rknn, pointer_head.rknn) not found in model directory"
            }), 404

        # Create GUI Actor helper
        from rkllama.api.rknnlite import GuiActorHelper
        gui_helper = GuiActorHelper(vision_encoder_path, pointer_head_path)

        try:
            # Process image with vision encoder
            img_vec_output = gui_helper.get_img_vec(image)
            if img_vec_output is None:
                return jsonify({"error": "Failed to process image with vision encoder"}), 500

            # Store in helper for later use
            gui_helper.img_vec_output = img_vec_output
            logger.debug(f"Vision encoder output shape: {img_vec_output.shape}")

            # TODO: Call RKLLM model with multimodal input
            # For now, we simulate the model run and use threading pattern
            run_args = (prompt,)
            run_kwargs = {"image": image}

            # Placeholder: In a full implementation, you would call the actual RKLLM model here
            # This would populate variables.global_gui_actor_result with hidden states
            # For now, we return an error indicating incomplete implementation

            logger.warning("GUI Actor RKLLM model integration pending - returning placeholder coordinates")

            # If we have hidden states from the model, use them
            global_gui_actor_result = variables.global_gui_actor_result
            if global_gui_actor_result is not None:
                output_image, px, py = gui_helper.run_pointer_head(global_gui_actor_result, image, label)
            else:
